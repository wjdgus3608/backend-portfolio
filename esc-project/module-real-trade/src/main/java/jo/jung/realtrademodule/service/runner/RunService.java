package jo.jung.realtrademodule.service.runner;
...
//          if (true){
    ...
    Flux<Stock> sellTargets = getSellTargets();
            ...
            if (cachedStocks == null) {
                return getBuyTargets()
                        .collectList()
                        .flatMap(list -> {
                            if (!list.isEmpty()) {
                                cachedStocks = Flux.fromIterable(list).cache();
                            }
                            return execute(sellTargets);
                        });
            }
...
//                        log.info("bSet.size() : " + bSet);
    ...
    private Flux<Stock> getSellTargets(){
        return accountService.getAccount()
                .filter(stock -> stock.getHasAmount()>0);
    }
    ...
    public boolean isBeforeBuyTime(){
        LocalTime now = LocalTime.now();
        LocalTime start = LocalTime.of(9, 00, 05);  // 오전 9시 00분 45초
