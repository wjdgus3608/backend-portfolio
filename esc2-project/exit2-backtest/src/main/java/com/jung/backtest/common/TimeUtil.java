...
public class TimeUtil {
    ...
    public static boolean isNMinutesPassed(String targetTime, String nowTime, int overMinute) {
        // 문자열을 LocalDateTime으로 변환
        LocalDateTime target = LocalDateTime.parse(targetTime, FORMATTER);
    ...
    public static String plusCandleTime(String currentTime, CandleTimeType candleTimeType){
        if(candleTimeType.equals(CandleTimeType.FIVE_MINUTE))
            return plusMinutes(currentTime, 5);
        else if(candleTimeType.equals(CandleTimeType.FIFTEEN_MINUTE))
            return plusMinutes(currentTime, 15);
        else if(candleTimeType.equals(CandleTimeType.ONE_HOUR))
            return plusMinutes(currentTime, 60);
        return currentTime;
    }

    public static boolean isFifteenMinuteClose(String time) {
        // time format: yyyyMMddHHmmss
        int minute = Integer.parseInt(time.substring(10, 12));
        int second = Integer.parseInt(time.substring(12, 14));

        // 15분봉은 00, 15, 30, 45 분의 00초에 마감
        return (minute % 15 == 0) && second == 0;
    }

    public static boolean isNMinuteClose(String time, int n) {
        // time format: yyyyMMddHHmmss
        int minute = Integer.parseInt(time.substring(10, 12));
        int second = Integer.parseInt(time.substring(12, 14));

        // N분봉은 0초에 N분 간격으로 마감
        return (minute % n == 0) && second == 0;
    }
}
